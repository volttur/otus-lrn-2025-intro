...
@pytest.mark.parametrize(('a', 'area'), [(5, 25), (5.5, 30.25)], ids=['integer', 'float'])
def test_square_area_positive(a, area):
    r = Square(a)
    assert r.area == area, f'Square area with side 5 has to be 25'

@pytest.mark.parametrize(('a', 'perimeter'), [(5, 20), (5.5, 22)], ids=['integer', 'float'])
def test_square_perimeter_positive(a, perimeter):
    r = Square(a)
    assert r.perimeter == perimeter, f'Square perimeter with side 5 has to be 20'

@pytest.mark.parametrize(('a', 'area_sum'), [(5, 40), (5.5, 45.25)], ids=['integer', 'float'])
...
@pytest.mark.parametrize(('a'), [('str'), (True), (None)], ids=['a str', 'a bool', 'a None'])
