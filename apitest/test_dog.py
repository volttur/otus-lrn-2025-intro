...
def test_random_edges_negative(amnt, expected, build_link_rnd):
    ...
    assert res.json().get('status') == stat_ok, f'Status for {amnt} imgs doesnt equal to "success"'
...
@pytest.mark.parametrize('breed', [utils.rnd_breed(utils.build_link(url_api, '/breeds/list/all')) for i in range(3)])
