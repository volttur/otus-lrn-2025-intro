...
def test_all_posts(build_link_all_posts):
    ...
    assert len(posts) == len(set([post["id"] for post in posts])), (
        "There are doubles in the list of ids"
    )
...
def test_get_post(id, build_link_all_posts):
    ...
    post = res.json()
    assert res.status_code == 200, "Status code doesn't equal 200"
    assert "list" not in str(type(post)), (
        "There are several posts with same id` in the result"
    )
