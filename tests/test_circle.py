...
@pytest.mark.parametrize(('r', 'area'), [(1, 3.14), (1.1, 3.8)], ids=['integer', 'float'])
def test_circle_area_positive(r, area):
    r = Circle(r)
    assert r.area == area, f'Circle area with sides 3 and 5 has to be 15'

@pytest.mark.parametrize(('r', 'perimeter'), [(2, 12.57), (2.5, 15.71)], ids=['integer', 'float'])
def test_circle_perimeter_positive(r, perimeter):
    r = Circle(r)
    assert r.perimeter == perimeter, f'Circle perimeter with sides 4 and 5 has to be 18'

@pytest.mark.parametrize(('r', 'area_sum'), [(1, 18.14), (4.5, 78.62)], ids=['integer', 'float'])
...
@pytest.mark.parametrize(('r'), [('str'), (True), (None)], ids=['r str', 'r bool', 'None'])
