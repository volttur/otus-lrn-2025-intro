...
def test_rectangle_area_positive(side_a, side_b, area):
    ...
    assert r.area == area, "Rectangle area with sides 3 and 5 has to be 15"


@pytest.mark.parametrize(
    ("a", "b", "perimeter"), [(4, 5, 18), (2.5, 7.5, 20)], ids=["integer", "float"]
)
def test_rectangle_perimeter_positive(a, b, perimeter):
    r = Rectangle(a, b)
    assert r.perimeter == perimeter, (
        "Rectangle perimeter with sides 4 and 5 has to be 18"
    )


@pytest.mark.parametrize(
    ("a", "b", "area_sum"), [(4, 5, 35), (4.5, 5.5, 39.75)], ids=["integer", "float"]
)
...
@pytest.mark.parametrize(
    ("a", "b"),
    [(0, 5), (5, 0), (-1, 5), (5, -1), (0, 0)],
    ids=["a zero", "b zero", "a -1", "b -1", "all zero"],
)
...
@pytest.mark.parametrize(
    ("a", "b"),
    [("str", 5), (5, "str"), (True, 5), (5, False), (None, None)],
    ids=["a str", "b str", "a bool", "b bool", "All None"],
)
...
@pytest.mark.parametrize(
    ("x"), [(5), ("str"), (True), (None)], ids=["int", "str", "bool", "None"]
)
